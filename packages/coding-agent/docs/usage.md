...
Use `/role create <name> <description>` to create a role profile, `/role <name>` to switch roles, `/role select` to pick from available roles, `/role memory` to inspect loaded role memory, `/role remember [--global|--project] <instruction>` to ask Pi to extract a candidate memory from the current conversation and confirm it in an editable dialog, and `/role save-memory` to save role-specific learnings from the current conversation. On TUI startup, Pi shows a lightweight notice when roles are available and no role is active. Role switching keeps the current session history and changes only the system prompt context and memory.
