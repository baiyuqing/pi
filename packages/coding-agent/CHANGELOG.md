...
- Added `/clear` and `/reset` slash commands to clear the terminal and start a fresh session, with optional `/clear <name>` to label the previous session for `/resume`. `/new` starts a fresh session without clearing terminal scrollback.
